...
const additems = async (req, res) => {
  const { productId, productName, category, Price, image, imageDetails, description, stock } =
    req.body;
...
const updateitems = async (req, res) => {
  ...
  const updateddata = await productdetails.findByIdAndUpdate(checkitems._id, update_data, {
    new: true,
  });
...
const deleteItems = async (req, res) => {
  ...
  const deletedata = await productdetails.findByIdAndDelete(id, req.body, { new: true });
